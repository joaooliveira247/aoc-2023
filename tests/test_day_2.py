...
def test_larger_values() -> None:
    test_game: str = "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green"
    assert larger_values(test_game) == [4, 2, 6]


def test_mult_list() -> None:
    test_values: list[int] = [2, 4, 6]
    assert mult_list(test_values) == 48
...
def test_day_2() -> None:
    ...
    assert sum([mult_list(larger_values(i)) for i in input]) == 2286
